...
function clean_xml_files($dir)
{
//delete google maps xml files that are more that 1 hour old
...
while ($file = readdir($dir_handle)) 
{
	...
	if (is_file($dir . $file))
	{
		...
		if (($now - filemtime($dir . $file))>3600)
		{
			unlink($dir . $file);
		}
	}
...
function wats_list_tickets($filtercategory, $catlist, $view, $idtype, $idpriority, $idstatus, $idowner, $idauthor, $idauthormetavalue)
{
	...
	//clean up old (> 1 hour) xml files generated for google maps
	clean_xml_files("/home/pompeybu/public_html/iidb/xml/");

	$output = "";
	if ($view == 0)
	{
//		$output .= "<table width=\"95%\"><tr><td style=\"align: left; border 0; background: none;\">\n";
...
//	$output .= "</td><td style=\"align: center; border 0; background: none; width: 200px\">\n";
	$xml_filename = uniqid() . ".xml";
	$map_uri = "http://www.pompeybug.co.uk/iidb/bigmap.html?" . basename($xml_filename);
	$xml_filename = "/home/pompeybu/public_html/iidb/xml/" . $xml_filename;
	$output .= "<a href=\"" . $map_uri . "\">";
//	$output .= "<img style=\"align: center;\" src=\"http://www.pompeybug.co.uk/iidb/wp-content/uploads/2010/02/Gmaps_icon.png\" title=\"Click to see selected issues on a map\">";
	$output .= "\n<P style=\"font-size: 1.2em; vertical-align: middle; align: right;\"><img style=\"vertical-align: middle;\" src=\"http://www.pompeybug.co.uk/icons/map_icon.jpg\">&nbsp;Click to see tickets on a map</P></a>\n";
//	$output .= "</td></tr></table>\n";
	...
	if ($wats_settings['meta_column_ticket_listing'] == 1 && $current_user->user_level == 10)
		...
		$colspan = 9;
    //create xml for google maps display
	...
	foreach ($tickets as $ticket)
	{
		...
		$newnode->setAttribute("title",htmlspecialchars(stripslashes($ticket->post_title)));
		...
		$newnode->setAttribute("description", "Ticket type: " . wats_ticket_get_type($ticket) . ", priority: " . wats_ticket_get_priority($ticket));
		$newnode->setAttribute("uri", get_permalink( $ticket->ID ));
		$latlng = split(",", get_post_meta($ticket->ID, 'Start Location', true));
...
function wats_ticket_submit_form()
{
	...
	if ($wats_settings['frontend_submit_form_access'] == 1 || ($wats_settings['frontend_submit_form_access'] == 2 && is_user_logged_in()))
	{
		...
		else
		{
			...
			$output .= '<tr><td style="align: left; border 0; background: none;">'.__('Name', 'WATS').' '.__('(required)','WATS').' </td><td style="align: left; border 0; background: none;"><input type="text" name="name" id="name" value="" size="22" /></td></tr>' . "\n";
			$output .= '<tr><td style="align: left; border 0; background: none;">'.__('Mail', 'WATS').' '.__('(required)', 'WATS').' </td><td style="align: left; border 0; background: none;"><input type="text" name="email" id="email" value="" size="22" /></td></tr>' . "\n";
			$output .= '<tr><td style="align: left; border 0; background: none;">'.__('Website', 'WATS').' </td><td style="align: left; border 0; background: none;"><input type="text" name="url" id="url" value="" size="22" /></td></tr></table><br />' . "\n";
		}
		$output .= '<p align="left" class="wats_ticket_form_title">'.__('Ticket details','WATS').'</p>'. "\n";

		$output .= '<table style="border: 0;" width="95%"><tr><td style="align: left; border 0; background: none; vertical-align: top;">' . "\n";
		$output .= '<p align="left">'.wats_ticket_details_meta_box($post,1).'</p>'. "\n";
		$output .= '<p align="left">Start Location : <input type="text" name="start_location" id="start_location" value="" size="22" /></p>'. "\n";
		$output .= '<p align="left">End Location : <input type="text" name="end_location" id="end_location" value="" size="22" /></p>'. "\n";
		$output .= '<P style="align: left; font-size: 0.9em;">Enter the co-ordinates of the issue if you know them or use the map on the right to find and automatically enter them.</P></td>'. "\n";
		$output .= '<td style="width: 320px; align: left; border 0; background: none;">'. "\n";
		$output .= '<div style="width: 300px; height: 300px;" id="map"></div>'. "\n";
		$output .= '<p style="align: left; font-size: 0.9em;">Use the pan and zoom tools to find the desired location on the map.  Once found click on the map once to choose start location and again to choose end location (if desired).  Once selected the locations can be moved by dragging.</p>'. "\n";
		$output .= '</td></tr></table>' . "\n";
	...
	if ($_POST[view] == 1 && ($wats_settings['frontend_submit_form_access'] == 1 || ($wats_settings['frontend_submit_form_access'] == 2 && is_user_logged_in())))
	{
		...
		else
		{
			...
			else
			{
				...
				add_post_meta($post_ID,'Start Location', $_POST[start_location]);
				add_post_meta($post_ID,'End Location', $_POST[end_location]);
